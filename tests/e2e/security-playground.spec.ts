...
test.describe("Security Playground", () => {
  ...
  test("loads default challenge and UI elements", async ({ page }) => {
    await expect(page.getByText("System Override Attempt")).toBeVisible();
  ...
  test("runs a simulation and updates results", async ({ page }) => {
    ...
    // Wait for analysis (simulated delay)
    await expect(runButton).toContainText("Analyzing...");
    await expect(runButton).toContainText("Run simulation", { timeout: 5000 });
  ...
  test("switching challenges updates context", async ({ page }) => {
    const challengeBtn = page.getByRole("button", { name: "Tool Exfiltration Attempt" });
    await challengeBtn.click();

    await expect(page.getByRole("heading", { name: "Tool Exfiltration Attempt" })).toBeVisible();
