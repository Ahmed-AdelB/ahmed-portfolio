  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const prsRes = await fetch(`https://api.github.com/search/issues?q=author:${username}+type:pr+is:merged`, { headers });
        const prsData = prsRes.ok ? await prsRes.json() : { total_count: FALLBACK_STATS.prs };
        ...
        const contributionsCount = commitsCount + prsData.total_count + (userData.public_repos * 5); // Rough heuristic if real data fails

        const newStats = {
          repos: userData.public_repos,
          commits: commitsCount,
          prs: prsData.total_count || FALLBACK_STATS.prs,
          contributions: contributionsCount > 100 ? contributionsCount : FALLBACK_STATS.contributions
        };
