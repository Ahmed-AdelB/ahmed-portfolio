...
import { GitPullRequest, GitMerge, AlertCircle, Clock, ExternalLink, ChevronDown, ChevronUp } from 'lucide-react';
...
export const ContributionTimeline: React.FC<Props> = ({ data }) => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Merged': return 'text-purple-600 bg-purple-50 dark:bg-purple-900/20 dark:text-purple-400 border-purple-200 dark:border-purple-800';
      case 'Open': return 'text-green-600 bg-green-50 dark:bg-green-900/20 dark:text-green-400 border-green-200 dark:border-green-800';
      case 'Closed': return 'text-red-600 bg-red-50 dark:bg-red-900/20 dark:text-red-400 border-red-200 dark:border-red-800';
  ...
  const getIcon = (status: string) => {
    switch (status) {
      case 'Merged': return <GitMerge size={18} />;
      case 'Open': return <GitPullRequest size={18} />;
      case 'Closed': return <AlertCircle size={18} />;
      ...
      <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 md:before:mx-auto md:before:translate-x-px md:before:h-full md:before:w-0.5 md:before:bg-gradient-to-b md:before:from-transparent md:before:via-zinc-200 md:before:to-transparent dark:md:before:via-zinc-800">
        <AnimatePresence mode='popLayout'>
          {filteredData.map((item, index) => (
            ...
            >
              {/* Timeline Dot/Icon */}
              <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-white dark:border-zinc-950 shadow-md shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 ${
                item.status === 'Merged' ? 'bg-purple-100 text-purple-600 dark:bg-purple-900 dark:text-purple-300' : 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800'
              }`}>
                {getIcon(item.status)}
              </div>
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)]">
                <div 
                  ...
                  onClick={() => setExpandedId(expandedId === item.pr_url ? null : item.pr_url)}
                >
                  ...
                  <div className="flex items-center gap-3 text-xs mb-3">
                    <span className={`px-2 py-0.5 rounded-full border ${getStatusColor(item.status)}`}>
                      {item.status}
                    </span>
                    <span className="text-green-600 dark:text-green-500 font-mono">+{item.additions}</span>
                    <span className="text-red-600 dark:text-red-500 font-mono">-{item.deletions}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-zinc-100 dark:border-zinc-800">
                    ...
                    {expandedId === item.pr_url ? <ChevronUp size={16} className="text-zinc-400" /> : <ChevronDown size={16} className="text-zinc-400" />}
                  </div>
