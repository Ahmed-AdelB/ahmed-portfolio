...
for (const slug of slugs) {
  ...
  const scripts = Array.from(document.querySelectorAll('script'))
    ...
    .filter(Boolean);
  const copyScript = scripts.find((script) => script.includes('COPY_RESET_DELAY'));
  if (!copyScript) {
  ...
  const passed = codeBlocks.length === buttons.length && codeBlocks.length === wrapped.length;
  if (!passed) allPass = false;
  console.log(`${slug}: code blocks=${codeBlocks.length}, buttons=${buttons.length}, wrapped=${wrapped.length} => ${passed ? 'PASS' : 'FAIL'}`);
}
